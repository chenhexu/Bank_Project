    ...
    <div className={`min-h-screen flex items-center justify-center bg-gradient-to-br ${isDarkMode ? 'from-gray-900 via-gray-800 to-gray-900' : 'from-blue-100 via-white to-blue-200'}`}>
      <div className={`${isDarkMode ? 'bg-gray-800/90' : 'bg-white/90'} shadow-xl rounded-3xl px-10 py-12 max-w-md w-full flex flex-col items-center`}>
        <div className="mb-8 flex flex-col items-center">
          <div className={`text-3xl font-extrabold mb-2 tracking-tight ${isDarkMode ? 'text-blue-400' : 'text-blue-700'}`}>Deposit Funds</div>
          <div className={`text-sm font-medium mb-2 ${isDarkMode ? 'text-blue-300' : 'text-blue-500'}`}>Add money to your BlueBank account.</div>
        </div>
        <form className="flex flex-col gap-4 w-full mt-4" onSubmit={handleDeposit}>
          ...
          />
          <button type="submit" className={`w-full py-3 rounded-xl font-semibold text-lg shadow transition mt-2 text-white ${isDarkMode ? 'bg-blue-500 hover:bg-blue-600' : 'bg-blue-600 hover:bg-blue-700'}`}>Deposit</button>
        </form>
